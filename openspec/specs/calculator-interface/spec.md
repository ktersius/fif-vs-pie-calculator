...
Defines the interactive single-page interface for selecting Historical Backtest or Monte Carlo Simulation, configuring applicable investor inputs, and presenting either single-period historical results or Monte Carlo distributions accessibly.
...
- Run count `5,000`, seed `42`, and mean block length `four years` SHALL be displayed as fixed assumptions rather than editable controls.
