# fif-pie-taxation Specification

## Purpose

Defines the New Zealand tax treatment applied to each portfolio: the InvestNow PIE fund under the Fair Dividend Rate structure using the investor's PIR, and the IBKR direct-ETF portfolio under the Foreign Investment Fund (FIF) regime, including the de minimis threshold, FIF-exempt dividend-only taxation, and FDR/CV method selection.
