...
`runSimulation` returns, for each platform, an array of H + 1 yearly records (Year 0..H, where H is the configurable investment horizon, default 20) each capturing opening balance, contributions, growth, dividends, fees, tax, and closing balance, plus method-selection detail (FDR vs CV) for the tax drag chart and debugging. Aggregates are derived from these records. To power the year-by-year breakdown drill-down, each record SHALL also carry the intermediate tax sub-components (PIE: taxable income, PIR, tax owed; IBKR: cost base, de minimis status, gross dividends, FTC, FDR and CV method figures, selected method) and a representative per-order fee template (gross, FX fee, brokerage fee, net) with the order count and annual fee totals, plus a crash-year flag. Carrying these values on the record avoids recomputing them in the UI.

**Breakdown drill-down UX: inline accordion table.**
The per-year tax and per-order fee breakdowns are surfaced through an expandable year-by-year table (one row per year, each expanding inline) rather than a modal or slide-over. Inline expansion keeps neighbouring years visible for comparison, which is the primary use of reference detail; a modal would force one-year-at-a-time viewing. Because every periodic order within a year is identical, the fee panel shows a single representative order plus the count and annual totals instead of up to 52 duplicate rows. Clicking a tax-drag chart bar expands the corresponding row as a secondary affordance, and chart tooltips give at-a-glance figures. Alternative considered: a slide-over drawer — rejected for cross-year comparison; a flat 1000+ row per-order table — rejected as overwhelming.
