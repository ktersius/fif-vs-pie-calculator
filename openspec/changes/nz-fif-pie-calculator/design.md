...
The workspace is currently empty apart from the OpenSpec scaffolding and the source specification document. We are building a greenfield, client-only React single-page application that projects two NZ investment vehicles over 20 years and compares their fee and tax outcomes. All computation is deterministic given the inputs (with the exception of crash-year placement, which is randomised) and runs entirely in the browser — there is no backend, persistence, or external API.
...
- No support for changing the 20-year horizon or the fixed constants via the UI.
