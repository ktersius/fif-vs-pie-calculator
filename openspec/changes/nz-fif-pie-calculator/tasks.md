...
- [ ] 2.2 Create `src/lib/types.ts` with `SimulationInputs`, per-year `YearRecord`, per-platform result, and aggregate `Summary` types
...
- [ ] 5.1 Implement a seedable PRNG and deterministic crash-year selection: N distinct years in [1, 20] derived from a seed, shared by both portfolios, stable unless the seed changes, and resized (keep-and-add / trim) when N changes
...
- [ ] 5.5 Implement annual balance expansion (contribution → growth/crash → dividends → withholding → management fee), flooring the closing balance at $0
...
- [ ] 5.7 Apply Year 20 exit fees (InvestNow 0.50% sell; IBKR single FX + capped brokerage)
- [ ] 5.8 Expose a single `runSimulation(inputs)` returning per-year records for both platforms plus aggregates
...
- [ ] 7.1 Build input controls with specified ranges/options and defaults (initial investment, periodic contribution, frequency, market return, dividend yield, marginal rate, PIR, crash years)
- [ ] 7.2 Constrain numeric inputs to their min/max
...
- [ ] 8.1 Build the portfolio balance line/area chart (Year 0–20, one series per platform)
